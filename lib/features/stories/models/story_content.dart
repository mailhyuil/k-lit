...
class StoryContent {
  ...
  const StoryContent({required this.version, required this.bodyAr});

  /// 텍스트 파일에서 생성
  factory StoryContent.fromText(String text, int version) {
    return StoryContent(version: version, bodyAr: text);
  }
  ...
  /// 복사 생성자
  StoryContent copyWith({int? version, String? bodyAr}) {
    return StoryContent(
...
class StoryMeta {
  ...
  const StoryMeta({required this.version});

  factory StoryMeta.fromJson(Map<String, dynamic> json) {
    return StoryMeta(version: json['version'] as int? ?? 1);
  }
