...
void main() {
  ...
  group('Story Providers', () {
    ...
    test('storyProvider returns a story when it exists', () async {
      ...
      // ACT
      final storyResult = await container.read(
        storyByStoryIdProvider('story-1').future,
      );
    ...
    test('storyProvider returns null when it does not exist', () async {
      ...
      when(() => mockQueryBuilder.select(any())).thenReturn(mockQueryBuilder);
      when(
        () => mockQueryBuilder.eq('id', 'story-nonexistent'),
      ).thenReturn(mockQueryBuilder);
      ...
      // ACT
      final storyResult = await container.read(
        storyByStoryIdProvider('story-nonexistent').future,
      );
    ...
    test('collectionStoriesProvider returns a list of stories', () async {
      ...
      when(
        () => mockQueryBuilder.eq(
          'story_collections.collection_id',
          'collection-1',
        ),
      ).thenReturn(mockQueryBuilder);
      when(
      ...
      // ACT
      final storiesResult = await container.read(
        storiesByCollectionIdProvider('collection-1').future,
      );
